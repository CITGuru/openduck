...
- **Removed unnecessary `unsafe`** — `std::env::set_var` calls in `openduck-cli`
  were wrapped in `unsafe` blocks, which are not required in Rust edition 2021.
  Removed the `unsafe` wrappers; kept the comment documenting the single-thread
  invariant. Inspired by initial report from [@hobostay](https://github.com/hobostay)
  ([hobostay/openduck@f37ba17](https://github.com/hobostay/openduck/commit/f37ba17f779b4fb1927699eb7508624f769cc74e)).
